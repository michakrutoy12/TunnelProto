...
class Server:
    ...
    async def handler(self, reader, writer):
        ...
        try:
            while True:
                data = await self.read_packet(reader)
                if not data:
                    break
    ...
    async def handshake(self, reader, writer):
        ...
        try:
            ...
            uid, port = await self.register_new_client(reader, writer)
    ...
    async def register_new_client(self, reader, writer):
        ...
        try:
            uid = await self._allocate_client_uid(reader, writer, port)
        except Exception:
            async with self.ports_lock:
    ...
    async def _allocate_client_uid(self, reader, writer, port):
        async with self.conn_lock:
            for _ in range(65536):
    ...
    async def run(self):
        ...
        self.server = await asyncio.start_server(self.handler, *self.bind_addr)
