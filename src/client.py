...
class Client:
	def __init__(self, server_addr, local_addr, cert_path, check_hostname, client_id):
		...
		self.client_id = bytes.fromhex(client_id.replace('-', ''))
	...
	async def handshake(self):
		state = False
		try:
			await self.write(self.master_writer, [b'\x01', self.client_id])
			...
			state = False
			logging.error(f'Handshake unsuccessful: {err}')
	...
	async def run(self):
		...
		try:
			self.master_reader, self.master_writer = await asyncio.open_connection(*self.server_addr, ssl=ssl_context)
		except Exception as err:
